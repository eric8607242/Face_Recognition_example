...
class Trainer:
    ...
    def validate(self, model, val_loader):
        ...
        embeddings = np.zeros([len(val_loader), self.embeddings_size])
        idx = 0
        ...
        for step, (X, y) in enumerate(val_loader):
            ...
            embeddings[idx:idx+N] = outs.cpu()
            idx += N

        true_positive_rate, false_positive_rate, accuracy, best_threshold = evaluate(embeddings, labels, n_folds=5)

        self.logger.info("Evaluate : Accuracy : {}, True positive rate : {}, False positive rate : {}".format(true_positive_rate, false_positive_rate, accuracy))
